...
export function getPaginationParams(query: PaginationQuery) {
  ...
  // Cap raised to 200000 so the app's requirement / available-vehicle feeds can
  // request a very large page in one go instead of being stuck; other endpoints
  // still default to 20 when they don't pass a limit.
  const limit = Math.min(200000, Math.max(1, Number(query.limit) || 20));
